...
const COUNT = 70;
const CUP_POSITION = [0, 0, -0.5];
const SPREAD = [3, 3, 0.5];
const Y_OFFSET = 0;
const Z_BACK = 0.5;
const SPEED_TOWARD_CAMERA = 0.06;
const SPEED_LATERAL = 0.04;
const Z_FADE_START = 0.8;
const Z_RESPAWN = 1.5;
const SIZE = 0.6;
...
export function CupGlowParticles() {
  ...
  if (agesRef.current === null) {
    agesRef.current = new Float32Array(COUNT);
  }
  ...
  useFrame((_state, delta) => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const z = pos[i * 3 + 2];
      const fadeIn = Math.min(1, ages[i] / FADE_IN_SEC);
      if (z > Z_RESPAWN) {
        ...
        opacityArr[i] = Math.max(0, (1 - t) * fadeIn);
      } else {
        opacityArr[i] = fadeIn;
      }
    }
